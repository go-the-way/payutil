...
func (w *Client) PostWeChatAPISelf(ctx context.Context, bm pay.BodyMap, path string, tlsConfig *tls.Config) (bs []byte, err error) {
	return w.doProdPost(ctx, bm, path, tlsConfig)
}

// 授权码查询openid（正式）
// 文档地址：https://pay.weixin.qq.com/wiki/doc/api/wxpay_v2/open/chapter4_8.shtml
func (w *Client) AuthCodeToOpenId(ctx context.Context, bm pay.BodyMap) (wxRsp *AuthCodeToOpenIdResponse, err error) {
...
func (w *Client) DownloadBill(ctx context.Context, bm pay.BodyMap) (wxRsp string, err error) {
...
func (w *Client) DownloadFundFlow(ctx context.Context, bm pay.BodyMap) (wxRsp string, err error) {
...
func (w *Client) Report(ctx context.Context, bm pay.BodyMap) (wxRsp *ReportResponse, err error) {
...
func (w *Client) BatchQueryComment(ctx context.Context, bm pay.BodyMap) (wxRsp string, err error) {
...
func (w *Client) doSanBoxPost(ctx context.Context, bm pay.BodyMap, path string) (bs []byte, err error) {
...
// Post请求、正式
func (w *Client) doProdPost(ctx context.Context, bm pay.BodyMap, path string, tlsConfig *tls.Config) (bs []byte, err error) {
	var url = baseUrlCh + path
...
func (w *Client) doProdPostPure(ctx context.Context, bm pay.BodyMap, path string, tlsConfig *tls.Config) (bs []byte, err error) {
	var url = baseUrlCh + path
	httpClient := xhttp.NewClient()
	if w.IsProd && tlsConfig != nil {
		httpClient.SetTLSConfig(tlsConfig)
...
// Get请求、正式
func (w *Client) doProdGet(ctx context.Context, bm pay.BodyMap, path, signType string) (bs []byte, err error) {
	var url = baseUrlCh + path
	if bm.GetString("appid") == util.NULL {
		bm.Set("appid", w.AppId)
	}
