...
func TestV3TransactionH5(t *testing.T) {
	...
	bm.Set("appid", "appid").
		...
		Set("notify_url", "https://www.fmm.ink").
		SetBodyMap("amount", func(b pay.BodyMap) {
			b.Set("total", 1).
				Set("currency", "CNY")
		}).
		SetBodyMap("scene_info", func(b pay.BodyMap) {
			b.Set("payer_client_ip", "127.0.0.1").
				Set("device_id", "device_id").
				SetBodyMap("store_info", func(b pay.BodyMap) {
					b.Set("id", "id").
						Set("name", "腾讯大厦分店").
						Set("area_code", "440305").
						Set("address", "广东省深圳市南山区科技中一道10000号")
				}).
				SetBodyMap("h5_info", func(b pay.BodyMap) {
					b.Set("type", "Wap").
						Set("app_name", "王者荣耀").
