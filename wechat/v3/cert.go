...
func GetPlatformCerts(ctx context.Context, mchid, apiV3Key, serialNo, privateKey string) (certs *PlatformCertRsp, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[%w]: %+v", pay.SignatureErr, err)
	}
...
func (c *ClientV3) GetAndSelectNewestCert() (serialNo string, snCertMap map[string]string, err error) {
	...
	if certs.Code == Success && len(certs.Certs) > 0 {
		...
		if len(certs.Certs) == 1 {
			...
			if err != nil {
				return pay.NULL, nil, fmt.Errorf("time.ParseInLocation(%s, %s),err:%w", xtime.TimeLayout, formatExpire, err)
			}
			if time.Since(expireTime) > 0 {
				return pay.NULL, nil, fmt.Errorf("wechat platform API cert expired, expired time: %s", formatExpire)
			}
		...
		for _, v := range certs.Certs {
			...
			if err != nil {
				return pay.NULL, nil, fmt.Errorf("time.ParseInLocation(%s, %s),err:%w", xtime.TimeLayout, formatEffective, err)
			}
			expireTime, err := time.ParseInLocation(xtime.TimeLayout, formatExpire, time.Local)
			if err != nil {
				return pay.NULL, nil, fmt.Errorf("time.ParseInLocation(%s, %s),err:%w", xtime.TimeLayout, formatExpire, err)
	...
	// failed
	return pay.NULL, nil, fmt.Errorf("GetAndSelectNewestCert() failed or certs is nil: %+v", certs)
}
