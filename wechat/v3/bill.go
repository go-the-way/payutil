...
func (c *ClientV3) V3BillTradeBill(ctx context.Context, bm pay.BodyMap) (wxRsp *BillRsp, err error) {
	if bm != nil {
...
func (c *ClientV3) V3BillFundFlowBill(ctx context.Context, bm pay.BodyMap) (wxRsp *BillRsp, err error) {
	if bm != nil {
		if bm.GetString("bill_date") == util.NULL {
			now := time.Now()
			yesterday := time.Date(now.Year(), now.Month(), now.Day()-1, 0, 0, 0, 0, time.Local).Format(util.DateLayout)
...
func (c *ClientV3) V3BillEcommerceFundFlowBill(ctx context.Context, bm pay.BodyMap) (wxRsp *EcommerceFundFlowBillRsp, err error) {
	if bm != nil {
		if bm.GetString("bill_date") == util.NULL {
			now := time.Now()
			yesterday := time.Date(now.Year(), now.Month(), now.Day()-1, 0, 0, 0, 0, time.Local).Format(util.DateLayout)
...
func (c *ClientV3) V3BillSubFundFlowBill(ctx context.Context, bm pay.BodyMap) (wxRsp *BillRsp, err error) {
	if bm != nil {
		if bm.GetString("bill_date") == util.NULL {
			now := time.Now()
			yesterday := time.Date(now.Year(), now.Month(), now.Day()-1, 0, 0, 0, 0, time.Local).Format(util.DateLayout)
