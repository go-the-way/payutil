...
func NewClient(appid, privateKey string, isProd bool) (client *Client, err error) {
	if appid == util.NULL || privateKey == util.NULL {
		return nil, pay.MissAlipayInitParamErr
	}
...
func (a *Client) PostAliPayAPISelf(ctx context.Context, bm pay.BodyMap, method string, aliRsp interface{}) (err error) {
	var bs []byte
...
// Deprecated
// 推荐使用 RequestParam()
func (a *Client) GetRequestSignParam(bm pay.BodyMap, method string) (string, error) {
	return a.RequestParam(bm, method)
}

// RequestParam 获取支付宝完整请求参数包含签名
// 注意：biz_content 需要自行通过bm.SetBodyMap()设置，不设置则没有此参数
func (a *Client) RequestParam(bm pay.BodyMap, method string) (string, error) {
	var (
		bodyBs []byte
		err    error
		sign   string
	...
	if bzBody, ok := bz.(pay.BodyMap); ok {
...
func (a *Client) PostAliPayAPISelfV2(ctx context.Context, bm pay.BodyMap, method string, aliRsp interface{}) (err error) {
	var (
		bs, bodyBs []byte
	)
	// check if there is biz_content
...
// 向支付宝发送自定义请求
func (a *Client) doAliPaySelf(ctx context.Context, bm pay.BodyMap, method string) (bs []byte, err error) {
	var (
		url, sign string
	)
	bm.Set("method", method)
...
// 向支付宝发送请求
func (a *Client) doAliPay(ctx context.Context, bm pay.BodyMap, method string, authToken ...string) (bs []byte, err error) {
	var (
...
// 公共参数处理
func (a *Client) pubParamsHandle(bm pay.BodyMap, method, bizContent string, authToken ...string) (param string, err error) {
	pubBody := make(pay.BodyMap)
...
// 公共参数检查
func (a *Client) checkPublicParam(bm pay.BodyMap) {
	bm.Set("format", "JSON").
		Set("charset", a.Charset).
		Set("sign_type", a.SignType).
		Set("version", "1.0").
...
// 文件上传
func (a *Client) FileRequest(ctx context.Context, bm pay.BodyMap, file *util.File, method string) (bs []byte, err error) {
	var (
		bodyStr string
		bodyBs  []byte
		aat     string
