...
func GetRsaSign(bm pay.BodyMap, signType string, privateKey *rsa.PrivateKey) (sign string, err error) {
	var (
		h              hash.Hash
		hashs          crypto.Hash
		encryptedBytes []byte
...
func (a *Client) getSignData(bs []byte, alipayCertSN string) (signData string, err error) {
	...
	if alipayCertSN != "" {
		...
		if alipayCertSN != a.AliPayPublicCertSN {
			return pay.NULL, fmt.Errorf("[%w], 当前使用的支付宝公钥证书SN[%s]与网关响应报文中的SN[%s]不匹配", pay.CertNotMatchErr, a.AliPayPublicCertSN, alipayCertSN)
		}
		...
		return pay.NULL, fmt.Errorf("[%w], value: %s", pay.GetSignDataErr, str)
	}
	// 普通公钥模式
...
func VerifySign(alipayPublicKey string, notifyBean interface{}) (ok bool, err error) {
	...
	if reflect.ValueOf(notifyBean).Kind() == reflect.Map {
		if bm, ok = notifyBean.(pay.BodyMap); ok {
