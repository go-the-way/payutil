...
func (c *Client) SendStruct(v interface{}) (client *Client) {
	...
	if err != nil {
		c.err = fmt.Errorf("[%w]: %v, value: %v", pay.MarshalErr, err, v)
		return c
	}
	...
	case TypeXML, TypeUrlencoded, TypeForm, TypeFormData:
		...
		if err = json.Unmarshal(bs, &body); err != nil {
			c.err = fmt.Errorf("[%w]: %v, bytes: %s", pay.UnmarshalErr, err, string(bs))
			return c
		}
...
func (c *Client) SendBodyMap(bm map[string]interface{}) (client *Client) {
	...
	case TypeJSON:
		...
		if err != nil {
			c.err = fmt.Errorf("[%w]: %v, value: %v", pay.MarshalErr, err, bm)
			return c
		}
		c.jsonByte = bs
	case TypeXML, TypeUrlencoded, TypeForm, TypeFormData:
